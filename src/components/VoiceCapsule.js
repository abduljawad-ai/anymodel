/**
 * VoiceCapsule — audio playback UI with a waveform visualization.
 *
 * Build a capsule into a container:
 *   capsule.build(container, { src, raw, durationMs, text })
 *
...
export class VoiceCapsule {
  ...
  /* Decode audio into [0..1] amplitude bars. Uses raw bytes when available; falls back to fetch() for http(s). */
  async decodeWaveform(src, raw, count) {
    const ac = this.ctx();
    try {
      let buf = raw || null;
  ...
  build(container, opts) {
    ...
    dur.textContent = this.formatDuration((opts && opts.durationMs ? opts.durationMs : 0) / 1000);
    ...
    paint(this.placeholderBars());

    const load = async () => {
      if (loaded) return;
      loaded = true;
      const wb = await this.decodeWaveform(src, opts && opts.raw, BAR_COUNT);
      if (wb) paint(wb);
      audio = new Audio(src);
      audio.addEventListener("loadedmetadata", () => {
        if (isFinite(audio.duration) && audio.duration > 0) {
          liveDur = audio.duration;
          dur.textContent = this.formatDuration(liveDur);
        }
      });
    ...
    playBtn.addEventListener("click", () => {
      ...
      this.current = { audio, cap, playBtn };

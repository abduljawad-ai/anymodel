/**
 * RobotAvatar — animated SVG robot face component.
 *
 * States: idle (slow blink loop), thinking (eyes darting), speaking (mouth moves).
 *
 * Usage:
 *   const avatar = new RobotAvatar(deps);
 *   avatar.buildHero(containerEl);   // large hero on empty state
 *   avatar.buildInline(containerEl); // small 24px avatar on message row
 *   avatar.setState(el, 'idle' | 'thinking' | 'speaking');
 */
...
export class RobotAvatar {
  ...
  startBlinkLoop(container) {
    ...
    const doBlink = () => {
      ...
      // Only blink during idle/speaking, not mid-thinking
      if (state === "thinking") {
        scheduleNext();
        return;
      }
  ...
  buildHero(container) {
    if (!container) return;
    container.innerHTML = this.makeSVG(this.heroOpts());
    container.classList.add("robot-container", "robot-hero");
    this.setState(container, "idle");
  ...
  buildInline(container) {
    if (!container) return;
    container.innerHTML = this.makeSVG(this.inlineOpts());

...
def write_icons_js() -> None:
    """Bundle every icon as a one-line string in js/icons.js with a
    synchronous `icon(name, className)` helper. Loaded before all other
    component scripts, so the helper is available the moment anything
    renders — zero async, no fetch, no shadow DOM."""
    lines = [
        "/* js/icons.js — auto-generated by tools/make_icons.py. Do not edit manually. */",
        "const ICON_MAP = {",
    ]
    for name in ICONS:
        lines.append(f"  {name}: `{render_svg(name)}`,")
    lines += [
        "};",
        "",
    ...
    js = "\n".join(lines)
    js_path = ROOT / "js" / "icons.js"
    js_path.parent.mkdir(parents=True, exist_ok=True)
    js_path.write_text(js, encoding="utf-8")
    print(f"Wrote {js_path.relative_to(ROOT)} ({len(js.encode('utf-8'))} bytes, {len(ICONS)} icons)")
